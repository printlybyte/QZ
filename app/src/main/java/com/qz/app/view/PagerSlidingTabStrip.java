...
//				tab.getPaint().setFakeBoldText(true);
				...
				// setAllCaps() is only available from API 14, so the upper case is made manually if we are on actionbar_background
				// pre-ICS-build
				if (textAllCaps) {
	...
	protected void onDraw(Canvas canvas) {
		...
		float lenth = ((TextView)currentTab).getPaint().measureText(((TextView)currentTab).getText().toString());
	    float space=	((lineRight-lineLeft)-lenth)/2;

		canvas.drawRect(   lineLeft+space, height - indicatorHeight, lineRight-space, height, rectPaint);
